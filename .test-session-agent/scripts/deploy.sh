...
# Deploy test-session-agent to Amazon EKS
#
# Usage:
...
# Deploy using Helm
echo "Deploying test-session-agent to namespace ${NAMESPACE} in environment ${ENVIRONMENT}..."
helm upgrade --install test-session-agent deployment/helm \
  --namespace ${NAMESPACE} \
  --create-namespace \
  --values ${VALUES_FILE}
